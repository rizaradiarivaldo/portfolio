    ...
    <motion.main className="relative flex min-h-screen flex-col items-center justify-center bg-background px-4 py-8 text-foreground" variants={containerVariants} initial="hidden" animate="visible">
      ...
      <div className="flex w-full max-w-4xl flex-1 flex-col items-center justify-center">
        ...
        <motion.p className="mb-12 max-w-2xl text-balance text-center text-lg text-muted-foreground md:text-xl" variants={itemVariants}>
          Write to me, I will tell you in detail about me. feel free to contact me regarding collaboration, projects, or just to say hi!
        </motion.p>

        {/* Contact Methods */}
        <motion.div className="grid w-full max-w-3xl grid-cols-1 gap-6 md:grid-cols-3 md:gap-8" variants={itemVariants}>
          {/* Telegram */}
          <a href="https://t.me/rizaradiarivaldo" target="_blank" rel="noopener noreferrer" className="group flex flex-col items-center gap-3 rounded-2xl border border-border/50 bg-card/30 p-6 backdrop-blur-sm transition-all hover:border-primary/50 hover:bg-card/50">
          ...
          {/* Mail */}
          <a href="mailto:rizaradiarivaldo@gmail.com" className="group flex flex-col items-center gap-3 rounded-2xl border border-border/50 bg-card/30 p-6 backdrop-blur-sm transition-all hover:border-primary/50 hover:bg-card/50">
            <div className="rounded-full bg-primary/10 p-3 transition-colors group-hover:bg-primary/20">
              <Mail className="h-6 w-6 text-primary" />
            </div>
            <div className="text-center">
              <div className="mb-1 text-sm text-muted-foreground">Mail</div>
              <div className="font-medium text-foreground">rizaradiarivaldo@gmail.com</div>
            </div>
          </a>

          {/* LinkedIn */}
          <a href="https://linkedin.com/in/rizaradiarivaldo" target="_blank" rel="noopener noreferrer" className="group flex flex-col items-center gap-3 rounded-2xl border border-border/50 bg-card/30 p-6 backdrop-blur-sm transition-all hover:border-primary/50 hover:bg-card/50">
            <div className="rounded-full bg-primary/10 p-3 transition-colors group-hover:bg-primary/20">
