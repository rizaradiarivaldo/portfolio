    ...
    <motion.section className="relative flex min-h-screen flex-col items-center justify-between px-6 py-12 md:px-12 lg:px-24" variants={containerVariants} initial="hidden" animate="visible">
      ...
      <div className="flex flex-1 flex-col items-center justify-center gap-8 text-center">
        ...
        <motion.div className="relative h-32 w-32 overflow-hidden rounded-full border-2 border-primary/20 shadow-2xl md:h-40 md:w-40" variants={itemVariants}>
          <Image src="/images/rizar.png" alt="Andrew Alexeyev - Senior Product Designer" fill className="object-cover" priority />
        </motion.div>

        {/* Name and Role */}
        <motion.div className="space-y-3" variants={itemVariants}>
          <h1 className="text-balance text-4xl font-bold tracking-tight text-foreground md:text-5xl lg:text-6xl">Andrew Alexeyev</h1>
          <p className="text-pretty text-lg text-muted-foreground md:text-xl lg:text-2xl">Senior Product Designer</p>
        </motion.div>
      </div>
      ...
      <div className="flex w-full flex-col items-center gap-8">
        ...
        <motion.div className="flex items-center gap-6" variants={itemVariants}>
          <Link href="https://www.linkedin.com/in/idrewha" target="_blank" rel="noopener noreferrer" className="group flex items-center gap-2 text-sm text-muted-foreground transition-colors hover:text-primary md:text-base" aria-label="Connect on LinkedIn">
